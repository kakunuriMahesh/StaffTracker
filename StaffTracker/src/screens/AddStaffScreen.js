...
export default function AddStaffScreen({ navigation }) {
  ...
  const save = async () => {
    ...
    if (!salary.trim()) { Alert.alert('Missing', 'Please enter salary amount.'); return; }
  ...
  const getDurationDisplay = () => {
    switch (salaryType) {
      ...
      case 'manual': return `${dayjs(salaryStartDate).format('DD MMM')} - ${dayjs(salaryEndDate).format('DD MMM YYYY')}`;
      default: return '';
    }
  };
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.roleContainer}>
            {ROLES.map(r => (
              <TouchableOpacity 
                ...
                style={[styles.roleChip, position === r && styles.roleChipActive]}
              >
                <Ionicons 
                  name={position === r ? 'checkmark-circle' : 'ellipse-outline'} 
                  size={16} 
                  color={position === r ? '#fff' : '#6B7280'} 
                  style={{ marginRight: 4 }}
                />
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.durationContainer}>
            {DURATION_TYPES.map(type => (
              ...
              >
                <Ionicons 
                  name={salaryType === type.key ? 'radio-button-on' : 'radio-button-off'} 
                  size={18} 
                  color={salaryType === type.key ? '#2563EB' : '#6B7280'} 
                />
            ...
            <View style={styles.dateField}>
              ...
              <View style={styles.inputWrapper}>
                ...
                <TextInput 
                  ...
                  onChangeText={setSalaryStartDate}
        ...
        <View style={styles.inputGroup}>
          ...
          {salaryType !== 'manual' && salary.length > 0 && (
            <Text style={styles.salaryHint}>{getDurationDisplay()}</Text>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  salaryHint: { fontSize: 12, color: '#059669', marginTop: 6 },
